...
#[derive(Logos, Clone, PartialEq, logos_display::Display)]
enum Token<'a> {
...
fn main() {
    ...
    let token_iter = Token::lexer(src)
        ...
        .map(|(tok, span)| match tok {
            ...
            // Ok(tok) => SimpleSpan::from(span).make_wrapped(tok),
            // Err(()) => SimpleSpan::from(span).make_wrapped(Token::Error),
        });
